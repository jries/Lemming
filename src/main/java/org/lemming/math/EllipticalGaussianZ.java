...
public class EllipticalGaussianZ implements OptimizationData {
		...
		return new MultivariateMatrixFunction() {
			...
			public double[][] value(double[] point) throws IllegalArgumentException {
				...
				for (int i = 0; i < xgrid.length; ++i) {
					...
					jacobian[i][INDEX_Z0] = point[INDEX_I0] * (dEsx(xgrid[i], point) * Ey(ygrid[i], point) * dSx(point[INDEX_Z0])
	...
	public double Sx(double z) {
		...
		return params[INDEX_WX] * Math.sqrt(1 + b * b + params[INDEX_AX] * b * b * b + params[INDEX_BX] * b * b * b * b);
	}

	public double Sy(double z) {
		double b = (z + params[INDEX_C] - params[INDEX_Mp]) / params[INDEX_D];
		return params[INDEX_WY] * Math.sqrt(1 + b * b + params[INDEX_AY] * b * b * b + params[INDEX_BY] * b * b * b * b);
	}
	...
	public double dSx(double z) {
		...
		value = 0.5 * params[INDEX_WX] * params[INDEX_WX] * (2 * b / d + 3 * A * b * b / d + 4 * B * b * b * b / d) / Sx(z);
		return value;
	}
	...
	public double dSy(double z) {
		...
		double d = params[INDEX_D];
		double b = (z + params[INDEX_C] - params[INDEX_Mp]) / d;
		value = 0.5 * params[INDEX_WY] * params[INDEX_WY] * (2 * b / d + 3 * A * b * b / d + 4 * B * b * b * b / d) / Sy(z);
		return value;
	}
